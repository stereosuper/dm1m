...
gulp.task('styles', function () {
    return gulp.src('src/scss/main.scss')
        ...
        .pipe(gulp.dest('dest/wp-content/themes/dm1m/css'))
        .pipe($.size({title: 'styles'}));
});

gulp.task('fonts', function() {
    return gulp.src('src/fonts/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/dm1m/fonts'))
        .pipe($.size({ title: 'fonts' }));
});

gulp.task('img', function() {
    return gulp.src('src/img/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/dm1m/img'))
        .pipe($.size({ title: 'img' }));
});

gulp.task('layoutImg', function() {
    return gulp.src('src/layoutImg/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/dm1m/layoutImg'))
...
gulp.task('js', function () {
    return browserify({
        ...
        //.pipe($.uglify())
        .pipe($.sourcemaps.write('./'))
        .pipe(gulp.dest('dest/wp-content/themes/dm1m/js'))
        .pipe($.size({ title: 'js' }));
});
...
gulp.task('theme', function() {
    return gulp.src('src/theme/**/*')
        .pipe(gulp.dest('dest/wp-content/themes/dm1m'))
...
gulp.task('wp', function() {
    WP.discover({path: 'dest'}, function( WP ){
        ...
        WP.theme.activate('dm1m', function( err, results ){
            console.log(err + results);
...
gulp.task('sitemap', function () {
    gulp.src('dest/wp-content/themes/dm1m/**/*.html', {
            read: false
        })
        .pipe(sitemap({
            siteUrl: 'http://dm1m.com'
        }))
        .pipe(gulp.dest('dest/wp-content/themes/dm1m'));
});
