...
// Disable Tags
function dm1m_unregister_tags(){
    unregister_taxonomy_for_object_type('post_tag', 'post');
}
add_action( 'init', 'dm1m_unregister_tags' );
...
function dm1m_remove_comment_author_class( $classes ){
...
function dm1m_remove_submenus() {
  $page = remove_submenu_page( 'themes.php', 'themes.php' );
  remove_menu_page( 'edit-comments.php' );
}
add_action( 'admin_menu', 'dm1m_remove_submenus', 999 );
function dm1m_remove_top_menus( $wp_admin_bar ){
    $wp_admin_bar->remove_node( 'wp-logo' );
}
add_action( 'admin_bar_menu', 'dm1m_remove_top_menus', 999 );

// Enlever le lien par défaut autour des images
function dm1m_imagelink_setup(){
	if(get_option( 'image_default_link_type' ) !== 'none') update_option('image_default_link_type', 'none');
}
add_action( 'admin_init', 'dm1m_imagelink_setup' );

// Add wrapper around iframe
function dm1m_iframe_add_wrapper( $return, $data, $url ){
    return "<div class='wrapper-video'>{$return}</div>";
}
add_filter( 'oembed_dataparse', 'dm1m_iframe_add_wrapper', 10, 3 );

// Enlever les <p> autour des images
function dm1m_remove_p_around_images($content){
   return preg_replace('/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content);
}
add_filter( 'the_content', 'dm1m_remove_p_around_images' );

// Allow svg in media library
function dm1m_mime_types($mimes){
    $mimes['svg'] = 'image/svg+xml';
    return $mimes;
}
add_filter( 'upload_mimes', 'dm1m_mime_types' );

// Custom posts in the dashboard
function dm1m_right_now_custom_post() {
...
add_action( 'dashboard_glance_items', 'dm1m_right_now_custom_post' );

// Add new styles to wysiwyg
function dm1m_button( $buttons ){
    array_unshift( $buttons, 'styleselect' );
    return $buttons;
}
add_filter( 'mce_buttons_2', 'dm1m_button' );
function dm1m_init_editor_styles(){
    add_editor_style();
}
add_action( 'after_setup_theme', 'dm1m_init_editor_styles' );

// Customize a bit the wysiwyg editor
function dm1m_mce_before_init( $styles ){
    $style_formats = array(
        array(
            'title' => 'Button',
            'selector' => 'a',
...
add_filter( 'tiny_mce_before_init', 'dm1m_mce_before_init' );

// Option page
function dm1m_menu_order( $menu_ord ){  
...
add_filter( 'custom_menu_order', 'dm1m_menu_order' );
add_filter( 'menu_order', 'dm1m_menu_order' );
...
add_filter( 'nav_menu_css_class', 'dm1m_css_attributes_filter' );


/*-----------------------------------------------------------------------------------*/
/* Sidebar & Widgets
/*-----------------------------------------------------------------------------------*/
function dm1m_register_sidebars(){
...
add_action( 'widgets_init', 'dm1m_register_sidebars' );

// Deregister default widgets
function dm1m_unregister_default_widgets(){
...
add_action( 'widgets_init', 'dm1m_unregister_default_widgets' );


/*-----------------------------------------------------------------------------------*/
/* Post types
/*-----------------------------------------------------------------------------------*/
// function dm1m_post_type(){
...
//     ));
// }
// add_action( 'init', 'dm1m_post_type' );

// function dm1m_taxonomies(){
...
/*-----------------------------------------------------------------------------------*/
function dm1m_scripts(){
    // header
	wp_enqueue_style( 'dm1m-style', get_template_directory_uri() . '/css/main.css', array(), DM1M_VERSION );

	// footer
	wp_deregister_script('jquery');
	wp_enqueue_script( 'dm1m-scripts', get_template_directory_uri() . '/js/main.js', array(), DM1M_VERSION, true );

    wp_deregister_script( 'wp-embed' );
}
add_action( 'wp_enqueue_scripts', 'dm1m_scripts' );


/*-----------------------------------------------------------------------------------*/
/* TGMPA
/*-----------------------------------------------------------------------------------*/
function dm1m_register_required_plugins(){
	$plugins = array(
        array(
